...
void populateImportant(string keywords){
    /*std::istringstream iss(keywords);
    for (wstring line; std::getline(iss, line); )
    {
        important.insert(line);
    }*/
}

int getEditDistance(string astOne, string astTwo, string keywords) {
    JSONValue * A = JSON::Parse(astOne.c_str());
    JSONValue * B = JSON::Parse(astTwo.c_str());

    JSONValue* rootA = A->AsObject().at(L"root");
    JSONValue* rootB = B->AsObject().at(L"root");

    populateImportant(keywords);
