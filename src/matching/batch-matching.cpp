...
int main(int argc, char** argv) {
  ...
  int rowblock = atoi(argv[5]);
  int colblock = atoi(argv[6]);
/*  
...
*/  
  ...
  for (int i = 0; i < blocksize; i++) {
    ...
    if (json[i] == NULL && !ifstream(str.str().c_str())) {
      cerr << "nonexistent file " << blocksize * rowblock + i << endl;
      maxA = i;
      break;
    } else if (json[i] == NULL) {
  ...
  for (int A = 0; A < maxA; A++) {
    for (int B = 0; B < maxB; B++) {
      ...
      if (matchtype == "pre") {
        ...
        if (ans[A][B] <= 10) cout << blocksize * rowblock + A << " " << blocksize * colblock + B << " " << ans[A][B] << endl;
      }
