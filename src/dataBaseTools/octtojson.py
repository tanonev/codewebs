...
def run():  
    ...
    logfilename = os.path.join(logdir,'log_' + str(hw_id) + '_' + str(part_id) + '.log')
    ...
    for submission_id,it in zip(submissions,range(len(submissions))):
        filePrefix = os.path.join(tmpdir,astfilePrefix + identstr(hw_id,part_id,submission_id))
    ...
    jsondirname = os.path.join(datadir,astfilePrefix + \
                '_' + str(hw_id) + '_' + str(part_id))
    os.system('mkdir ' + jsondirname) 
    cnt = 0
    for ast in sortedASTs:
        filePrefix = os.path.join(jsondirname,astfilePrefix + '_' + str(cnt))
    ...
    numAST = cnt
    output_ids = os.path.join(jsondirname,'submissionids.dat')
    ...
    os.system('rm -rf ' + os.path.join(datadir,astdirname))
    logging.debug('Done.')
